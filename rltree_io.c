/*
 * in/out function for rltree and rlquery
 * Teodor Sigaev <teodor@stack.net>
 * contrib/rltree/rltree_io.c
 */
#include "postgres.h"
...
PG_FUNCTION_INFO_V1(rltree_in);
PG_FUNCTION_INFO_V1(rltree_out);
PG_FUNCTION_INFO_V1(rlquery_in);
PG_FUNCTION_INFO_V1(rlquery_out);
...
rltree_in(PG_FUNCTION_ARGS)
{
	...
	result = (rltree *) palloc0(LTREE_HDRSIZE + totallen);
...
#define NEXTLEV(x) ( (rlquery_level*)( ((char*)(x)) + ITEMSIZE) )

Datum
rlquery_in(PG_FUNCTION_ARGS)
{
	char	   *buf = (char *) PG_GETARG_POINTER(0);
	char	   *ptr;
	int			num = 0,
	...
	curqlevel = tmpql = (rlquery_level *) palloc0(ITEMSIZE * num);
	ptr = buf;
	while (*ptr)
	{
		charlen = pg_mblen(ptr);
	...
	result = (rlquery *) palloc0(totallen);
	...
	while ((char *) curqlevel - (char *) tmpql < num * ITEMSIZE)
	{
		...
		if (curqlevel->numvar)
		{
			...
			while (lptr - GETVAR(curqlevel) < curqlevel->numvar)
			{
				...
				lrptr->val = rltree_crc32_sz(lptr->start, lptr->len);
...
Datum
rlquery_out(PG_FUNCTION_ARGS)
{
	rlquery	   *in = PG_GETARG_LQUERY(0);
	char	   *buf,
			   *ptr;
	int			i,
				j,
				totallen = 1;
	rlquery_level *curqlevel;
	rlquery_variant *curtlevel;

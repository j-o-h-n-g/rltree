/*
 * op function for rltree and rlquery
 * Teodor Sigaev <teodor@stack.net>
 * contrib/rltree/rlquery_op.c
 */
#include "postgres.h"
...
bool
			compare_subnode(rltree_level *t, char *qn, int len, int (*cmpptr) (const char *, const char *, size_t), bool anyend)
{
...
int
rltree_strncasecmp(const char *a, const char *b, size_t s)
{
...
static bool
checkLevel(rlquery_level *curq, rltree_level *curt)
{
	int			(*cmpptr) (const char *, const char *, size_t);
	rlquery_variant *curvar = LQL_FIRST(curq);
	int			i;

	for (i = 0; i < curq->numvar; i++)
	{
		cmpptr = (curvar->flag & LVAR_INCASE) ? rltree_strncasecmp : strncmp;
...
static bool
checkCond(rlquery_level *curq, int query_numlevel, rltree_level *curt, int tree_numlevel, FieldNot *ptr)
{
	uint32		low_pos = 0,
				high_pos = 0,
				cur_tpos = 0;
	int			tlen = tree_numlevel,
				qlen = query_numlevel;
	int			isok;
	rlquery_level *prevq = NULL;
	rltree_level *prevt = NULL;

 ...
 * contrib/rltree/rltxtquery_io.c
 */
#include "postgres.h"
...
PG_FUNCTION_INFO_V1(rltxtq_in);
PG_FUNCTION_INFO_V1(rltxtq_out);
...
pushval_asis(QPRS_STATE *state, int type, char *strval, int lenval, uint16 flag)
{
	...
	pushquery(state, type, rltree_crc32_sz(strval, lenval),
	...
	if (LTXTQUERY_TOO_BIG(state.num, state.sumlen))
		ereport(ERROR,
				(errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
				 errmsg("rltxtquery is too large")));
	commonlen = COMPUTESIZE(state.num, state.sumlen);

	query = (rltxtquery *) palloc(commonlen);
...
Datum
rltxtq_out(PG_FUNCTION_ARGS)
{
	rltxtquery  *query = PG_GETARG_LTXTQUERY(0);
	INFIX		nrm;
